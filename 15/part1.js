...
const drawGrid = () => {
  ...
  for (let y = minY; y <= maxY; y++) {
    ...
    for (let x = minX; x <= maxX; x++) {
      ...
      line.push(isDroid ? 'D' : isOrigin ? '0' : isVisited ? '+' : isWall ? '#' : ' ');
    }
...
const getDuplicateTiles = tiles => {
  const tileCounts = tiles.reduce(
    (acc, tile) => ({
      ...acc,
      [tile]: (acc[tile] || 0) + 1
    }),
  ...
  const tilesVisitedTwice = Object.keys(tileCounts).filter(k => tileCounts[k] > 1);
  return tilesVisitedTwice;
};

const findShortestPath = () => {
  const visitedTiles = [...state.droid.visited];
  let duplicateTiles = getDuplicateTiles(visitedTiles);

  while (duplicateTiles.length) {
    const firstIndex = visitedTiles.indexOf(duplicateTiles[0]);
    const lastIndex = visitedTiles.lastIndexOf(duplicateTiles[0]);

    visitedTiles.splice(firstIndex, lastIndex - firstIndex);
    duplicateTiles = getDuplicateTiles(visitedTiles);
  }

  state.minMoves = visitedTiles.length - 1;
};
...
run();
console.log(`The fewest movements required to reach the oxygen system is ${state.minMoves}`);
